...
const (
	OracleEventName           = "QueryInfo"
	OracleResponseBytesName   = "responseBytes"
	OracleResponseUint256Name = "responseUint256"
	MaxRetryTimes             = 3
...
func (e *EventWatch) subscribeEvent() error {
	fmt.Println("subscribeEvent is beaning!!!")
	query := ethereum.FilterQuery{
		Addresses: []common.Address{
			common.HexToAddress(e.Config.OracleContractAddress),
		},
		Topics: [][]common.Hash{
			{e.OracleABI.Events[OracleEventName].ID()},
		},
	}
...
func (e *EventWatch) dealQuery(vLog types.Log) error {
	...
	err := e.OracleABI.Unpack(queryInfo, OracleEventName, vLog.Data)
	if err != nil {
...
func (e *EventWatch) sendQueryResponse(res interface{}, stateCode uint64, queryInfo *OracleQueryInfo, resParamType string) error {
	...
	switch resParamType {
	case "bytes":
		responseName = OracleResponseBytesName
	case "uint256":
		responseName = OracleResponseUint256Name
	default:
